...
const int SIGNAL_THRESHOLD      = 600;    // MUUDETUD 50-LT. Min threshold to trigger on. See calibration.pdf for conversion to mV.
const int RESET_THRESHOLD       = 250;//MUUDETUD 25LT    
...
void setup() {
  ...
  Serial.println(F("##########################################################################################"));
  Serial.println(F("### CosmicWatch: The Desktop Muon Detector. Edition Sassu"));
  Serial.println(F("### Questions? saxani@mit.edu Modified by saskia.poldmaa@gmail.com"));
...
void loop()
{
  while (1){
    if (analogRead(A0) > SIGNAL_THRESHOLD){ 
      digitalWrite(3, HIGH); //LISATUD, MUUDETUD
      ...
      //temperatureC = (((analogRead(A3)+analogRead(A3)+analogRead(A3))/3. * (3300./1024)) - 500.)/10. ; MUUDETUD
